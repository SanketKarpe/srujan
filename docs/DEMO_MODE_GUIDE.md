...
## Option 2: Standalone Mock Server (Simplest)

If you don't want to run the full API, use the standalone mock server:

### Create `src/demo/mock_server.py`:
...
app = FastAPI(title="Srujan Demo Server")
...
async def get_threats():
    return {"threats": MOCK_DATA.get("threats", []), "total": len(MOCK_DATA.get("threats", []))}
...
async def get_ml_alerts():
    ...
    return {"alerts": alerts, "total": len(alerts), "by_risk_level": by_risk}
...
```bash
cd src/demo
python3 mock_data_generator.py  # Generate data
python3 mock_server.py  # Start server
```
